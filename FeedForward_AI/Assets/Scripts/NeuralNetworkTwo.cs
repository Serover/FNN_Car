...
public class NeuralNetwork
{
    ...
    public NeuralNetwork(int[] layer, float learningRate, float bias)
    {
        ...
        for (int i = 1; i < layer.Length; i++)
        {
            ...
            for (int j = 0; j < neurons[i].Length; j++)
            {
                ...
                for (int k = 0; k < neuronsPreviousLayer; k++)
                {
                    ...
                    double rDouble = (random.NextDouble() * 2) - 1.0;
                    neuronWeights[k] = (float)rDouble;
                }
                // 1D
    ...
    public void BackPropOutputLayer(float[] correctOutput)
    {
        ...
        for (int j = 0; j < neurons[outPutLayer].Length; j++)
        {
            ...
            {
                weights[outPutLayer - 1][j][k] -= (connectionCalc[outPutLayer][j] * neurons[outPutLayer - 1][k]) * learningRate;
            }
        }
    }
    ...
    public void BackPropMiddleLayer()
    {
        for (int i = layer.Length - 2; i > 0; i--)
        {
            ...
            for (int j = 0; j < neurons[i].Length; j++)
            {
                ...
                for (int k = 0; k < neurons[i - 1].Length; k++)
                {
                    weights[i - 1][j][k] -= (calc[j] * neurons[i - 1][k]) * learningRate;
                }
            }
